...
import com.akivamu.example.data.PetData;
...
public class PetResource extends AbstractResource {
    ...
    @ApiResponses({
        ...
        @ApiResponse(code = 400, message = "Invalid ID supplied", response = com.akivamu.example.models.ApiResponse.class),
        @ApiResponse(code = 404, message = "Pet not found", response = com.akivamu.example.models.ApiResponse.class)
    })
    ...
    public ResponseEntity<Pet> getPetById(@ApiParam(value = "ID of pet that needs to be fetched", allowableValues = "range[1,10]", required = true) @PathVariable("id") Integer petId) throws Exception {
        ...
        if(pet != null) {
            ...
            throw new NotFoundException(com.akivamu.example.models.ApiResponse.ERROR, "Pet " + petId + " not found");
        }
    }
}
